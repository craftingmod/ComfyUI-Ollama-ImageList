...
- Reference Director Image Editor now separates View, Crop, and Mask interactions. Crop-frame interior drags move the crop, exterior drags pan the image, Space-drag pans from anywhere, zoom-out stops at the initial fitted scale, and zoom-dependent Pan bounds keep the image covering the entire editor Stage so checkerboard space cannot appear. The editor also provides source-pixel integer fields, four-corner resizing, previewed optional `rembg` extraction, source-SHA foreground cache reuse, and immutable-original restoration.
