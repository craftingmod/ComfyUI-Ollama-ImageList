...
- Image Crop aspect presets for Custom, Original, 1:1, 4:3, 3:4, 3:2, 2:3, 16:9, and 9:16, including ratio-locked corner resizing and coupled pixel dimensions.
- Keep-mask editing now includes an Undoable Invert mask action, a screen-size-accurate circular Erase/Restore cursor with distinct colors, and a transient Alt modifier that reverses both the active brush and its cursor color.
...
- Newly added videos now default to **V** enabled and the separate derived **A** output disabled, while explicit saved toggles remain authoritative. IMAGE, VIDEO, and AUDIO card badges use distinct low-saturation colors for faster GridView scanning.
- Reference Director adds connectable Advanced inputs `limit_image_pixels`/`max_image_pixels` and `composite_alpha`/`alpha_background` at stable `widget_values[1]` through `[4]`. Limited performs aspect-preserving, downscale-only resizing; Opaque composites alpha-bearing IMAGE outputs onto the selected fallback color and emits RGB. Both run after per-item edits and before float32 tensor allocation while leaving sources, materialized edits, VIDEO, and AUDIO untouched; effective widget or upstream values drive execution caching and the manifest instead of Director state.
- Image, Audio, and Video editor backdrops now dismiss only untouched dialogs with neither Undo nor Redo available. The Image Editor hides numeric Viewport controls and places Reset view beside Undo/Redo.
- Applying an image edit now immediately retains and veils the existing thumbnail with a centered spinner while mask upload/materialization runs, prevents duplicate Edit entry, and dirties the ComfyUI canvas after the edited proxy replaces the card DOM so no card drag or reorder is required.
- The Image Editor now places Caption directly below the media viewport and stretches it through the remaining left-column height. Errors and Restore original/Cancel/Apply occupy the bottom of the right controls column, while Audio and Video Trim retain their single-column footer. The editor also uses a wider 1280px-responsive layout for landscape sources, excludes View-only Pan/Zoom changes from Undo history, and restores Interaction mode through Undo/Redo.
...
- Reference Director Edit now opens on media-area double-clicks only; card selection preserves the clicked DOM so native browser double-click detection remains intact, while Caption/footer double-clicks and the overlaid × remain excluded.
- Reference Director workflow restoration now renders all initial loading cards once, retains four-item runtime hydration concurrency, and batches progressive preview completions into one board render per animation frame.
- Reference Director Image Editor now defaults to View and separates View, Crop, and Mask interactions. A fully visible focused Crop frame supports movement and four-corner resizing; a selected frame clipped by the Viewport enters resize-only mode, where body drags pan without changing the source crop and only visible corner handles remain editable. Clicking outside fully unfocuses the frame, while a short click selects it again. Ctrl-drag pans from anywhere without relying on keyboard focus, zoom-out stops at the initial fitted scale, and zoom-dependent Pan bounds keep the image covering the entire editor Stage so checkerboard space cannot appear. Pointer behavior is resolved through explicit selection and gesture states instead of overlapping event-handler flags. The editor also provides source-pixel integer fields, four-corner resizing, previewed optional `rembg` extraction, source-SHA foreground cache reuse, and immutable-original restoration.
- A focused Crop frame that exactly fills the Viewport uses body drag for image Pan instead of an ineffective frame move, while retaining all four resize handles.
- `grid_columns`, float `preview_pixels` (MPixel), `show_captions`, `card_aspect`, and `waveform_pairs` are native V3 advanced inputs while remaining display-only for execution caching; hidden card captions stay editable in the detail dialogs.
- Image cards now show transparent pixels on a checkerboard, and every media section exposes an adaptive type-specific add control in the next free grid cell or as a full-width row when the grid is full.
- Portrait GridView media can no longer overflow its card before `object-fit` is applied, and `card_aspect` now includes 3:4 and 9:16 portrait presets.
- Audio media areas now remain at a fixed 16:9 ratio, while the native `preview_fit` Advanced proxy switches image/video GridView previews between `contain` and `cover` without affecting execution media.
- Audio/Video Trim now places Caption below the timeline and range controls, directly above a compact footer with range-specific Undo/Redo on the left and Cancel/Apply on the right.
- Video Trim now shows one seek-synchronized, sound-enabled video preview above a half-height waveform. Continuous seeks are limited to 100ms while gesture completion remains exact, and silent videos keep visual playback with a distinct No audio track timeline.
- Grid Edit actions now use accessible inline pencil SVGs instead of repeated text, and all-zero waveforms show a Silent center-line state in Grid and Trim while missing VIDEO audio tracks remain distinctly labeled and disabled.
