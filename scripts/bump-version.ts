...
export function incrementPatchVersion(pyprojectToml: string): VersionBump {
  ...
  return {
    currentVersion,
    newVersion,
    updatedToml: pyprojectToml.replace(projectSection, () => updatedProjectSection),
  }
}
...
async function main(): Promise<void> {
  ...
  const { currentVersion, newVersion, updatedToml } = incrementPatchVersion(pyprojectToml)
  ...
  if (wasClean) {
    ...
    console.log("Please manually commit and tag:")
