...
- **Reference Director** — adds local image, audio, and video files to equal, vertically stacked **Images / Videos / Audio** boards; keeps independent order and output toggles per media category without tab state; highlights drag reorder targets; collapses empty boards to compact rows; overlays every preview with a light original-filename gradient and full-name tooltip; moderately desaturates only the media visual for disabled outputs while keeping surrounding information and controls at full brightness; places per-card controls below optional caption fields; provides compact Audio auditioning, on-demand sound-enabled VIDEO range playback, and detailed waveform trim playback; preserves safe original upload names for detail views; exposes native advanced display proxies plus an optional exact-MPixel, downscale-only IMAGE output limit and Preserve/Opaque alpha policy with a selectable fallback background; attaches raw user captions; and applies source-pixel four-corner image crops, previewed optional `rembg` foreground extraction, erase/restore masks, flips, backgrounds, reversible **Restore original**, and per-item audio/video trims.
