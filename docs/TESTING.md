...
3. Enter unique captions, including a separate Audio caption for one video. Confirm every media preview shows the source filename in a compact bottom gradient with ellipsis and a full-name tooltip, transparent images use a checkerboard background, portrait previews remain contained without overflowing, Grid Edit uses a tooltip/accessible pencil icon, and detail editors also show the filename. Confirm an all-zero waveform shows Silent in Grid and Trim without disabling playback, while a VIDEO with no audio track shows No audio track and disabled Audio controls. Toggle `show_captions` off under **Show advanced inputs**, confirm the card fields collapse without losing text, and edit a caption through the pencil icon before showing the fields again. Change `card_aspect` through landscape and portrait values, switch `preview_fit` between contain and cover, and change `waveform_pairs`; confirm the native fields proxy into the Director UI while Audio media remains 16:9. Set `limit_image_pixels` to Limited and enter an exact `max_image_pixels` below one large source's MP value; confirm only that IMAGE output is downsized, with no upscaling of smaller images or changes to VIDEO/AUDIO. Set `composite_alpha` to Opaque, change `alpha_background`, and confirm transparent IMAGE outputs become RGB on that color without changing stored sources. Reorder Images, Videos, and Audio independently with drag/drop and the arrow controls, then toggle one card off in each channel. Confirm each section shows a tile add control when its last row has room and a full-width add control when the grid is full.
...
5. Use Grid play/stop on Audio cards, then open Edit and confirm Caption is immediately above the footer, with Undo trim/Redo trim on the left and Cancel/Apply on the right. Adjust both trim handles while checking the draft range with Play/Pause/Stop. Trim the audio and video to known second ranges, cancel once, then apply. Confirm the native AUDIO waveform/sample count and VIDEO duration match the selected ranges.
