...
| Public media scope | Ollama Generate exposes IMAGE; llama.cpp Generate exposes optional IMAGE and AUDIO |
...
- Phase 5: optional `llama-cpp-python` multimodal generation is implemented with IMAGE and AUDIO inputs, lazy dependency import, GGUF Combo discovery from ComfyUI's registered `LLM` paths (including `extra_model_paths.yaml`) plus the local `models/LLM` fallback, a typed sampling-preset connection, selectable MTMD handlers including Qwen 3 ASR, serialized execution, and unconditional per-request model cleanup.
...
The Ollama node intentionally exposes no audio input. Native llama.cpp support depends on a separately installed platform/Python/CUDA-compatible wheel and modality-specific GGUF/mmproj compatibility.
