...
The Ollama node intentionally exposes no audio or video input. Native llama.cpp support depends on a separately installed platform/Python/native-backend-compatible wheel, FFmpeg for VIDEO, and modality-specific GGUF/mmproj compatibility. Detailed operational documentation is in [`LLAMA_CPP.md`](LLAMA_CPP.md).
